...
void readCommand(COMMAND *c, const Grid *grid) {
    ...
    do {
        ...
        if (strcmp(token, "exit") == 0 || strcmp(token, "quit") == 0 || strcmp(token, "q") == 0) {
            ...
            if (c->command == COMMAND_KNIGHT && c->p1.row < MAX_ROWS && c->p1.col < MAX_COLS) {
                validCommand = 1;
            }
        }
        if (validCommand == 0) {
...
int main() {
    ...
    for (;;) {
        ...
        if (cmd.command == COMMAND_EXIT) {
            ...
            Grid grid_knight = {0};
            grid_knight.rows = cmd.p1.row;
            grid_knight.cols = cmd.p1.col;

            grid2 = grid_knight;
            grid_to_graph(&grid_knight, &graph, true);
